...
const Dbms = () => {
  ...
  const handleCreate = async () => {
    ...
    const newNote = await createDbmsNote(title);
    setShowCreate(false);
    setTitle("");
    navigate(`/dbms/${newNote._id}`);
  };
        ...
        <div className="flex flex-1 gap-8 px-10 py-8">
          ...
          <div className="flex-1">
            ...
            {filteredNotes.length === 0 ? (
              <p className="text-gray-400 mt-10">No DBMS notes yet.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-6">
                {filteredNotes.map((n) => (
                  <div
                    key={n._id}
                    onClick={() => navigate(`/dbms/${n._id}`)}
        ...
        {showCreate && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-[#0f1422] border border-white/10 rounded-2xl w-[420px] p-6 shadow-xl">
              <h2 className="text-lg font-semibold mb-6">Create DBMS Note</h2>

              <input
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder="Eg: Normalization, Indexing..."
