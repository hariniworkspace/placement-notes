...
const Oops = () => {
  ...
  const handleCreate = async () => {
    ...
    const newNote = await createOopsNote(title);
    setShowCreate(false);
    setTitle("");
    navigate(`/oops/${newNote._id}`);
  };
      ...
      <div className="flex-1 ml-64 flex flex-col">
        ...
        <div className="flex flex-1 gap-8 px-10 py-8">
          ...
          <div className="flex-1">
            ...
            {filteredNotes.length === 0 ? (
              <p className="text-gray-400 mt-10">No OOPS notes yet.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-6">
                {filteredNotes.map((n) => (
                  <div
                    key={n._id}
                    onClick={() => navigate(`/oops/${n._id}`)}
            ...
            <div className="space-y-3 max-h-[360px] overflow-y-auto pr-1">
              {revisionNotes.map((n) => (
                ...
                >
                  ...
                  >
                    <p className="text-sm font-medium text-white hover:text-purple-400">
                      {n.title}
                    </p>
                  </div>
                </div>
        ...
        <div className="fixed bottom-8 right-8 z-50">
          <button
            ...
            className="flex items-center gap-3 px-7 py-3.5 rounded-xl text-white font-semibold text-base bg-gradient-to-r from-fuchsia-600 via-purple-600 to-indigo-600 shadow-2xl border border-white/10 hover:scale-105 transition-all"
          >
            <Plus size={18} />
            New Note
          </button>
