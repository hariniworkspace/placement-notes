  ...
  const editor = useEditor(
    {
      extensions: [
        ...
        Placeholder.configure({
          placeholder: "Write your OOPS notes for this section...",
        }),
      ],
  ...
  return (
    <div className="bg-[#111827] border border-white/10 rounded-lg p-5 relative group">
      ...
      <input
        ...
        placeholder="Eg: Definition / Example / Diagram / Key Points"
...
const OopsDetail = () => {
  ...
  useEffect(() => {
    (async () => {
      const data = await getOopsNoteById(id);
      setNote(data);
    })();
  }, [id]);
  ...
  const deleteBlock = (blockId) => {
    const filtered = note.blocks.filter((b) => b._id !== blockId);
    setNote({ ...note, blocks: filtered });
  };

  const handleSave = async () => {
    await updateOopsNote(id, {
      blocks: note.blocks,
